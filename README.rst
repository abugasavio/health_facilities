...
Mapping Health Facilities in Kenya
...
health_facilities relies extensively on environment settings which **will not work with Apache/mod_wsgi setups**. It has been deployed successfully with both Gunicorn/Nginx and even uWSGI/Nginx.

For configuration purposes, the following table maps the 'health_facilities' environment variables to their Django setting:
...
DJANGO_DEFAULT_FROM_EMAIL               DEFAULT_FROM_EMAIL          n/a                                            "health_facilities <noreply@abugasav.io>"
DJANGO_SERVER_EMAIL                     SERVER_EMAIL                n/a                                            "health_facilities <noreply@abugasav.io>" 
DJANGO_EMAIL_SUBJECT_PREFIX             EMAIL_SUBJECT_PREFIX        n/a                                            "[health_facilities] "
======================================= =========================== ============================================== ======================================================================
...
.. code-block:: bash
    ...
    cd health_facilities
    celery -A health_facilities.taskapp worker -l info
...
To run the tests, enter the health_facilities/tests directory and run the following commands::
...
..  code-block:: bash
    ...
    git remote add dokku dokku@yourservername.com:health_facilities
    git push dokku master
    ssh -t dokku@yourservername.com dokku redis:create health_facilities-redis
    ssh -t dokku@yourservername.com dokku redis:link health_facilities-redis health_facilities
    ssh -t dokku@yourservername.com dokku postgres:create health_facilities-postgres
    ssh -t dokku@yourservername.com dokku postgres:link health_facilities-postgres health_facilities
    ssh -t dokku@yourservername.com dokku config:set health_facilities DJANGO_SECRET_KEY=RANDOM_SECRET_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set health_facilities DJANGO_SETTINGS_MODULE='config.settings.production'
    ssh -t dokku@yourservername.com dokku config:set health_facilities DJANGO_AWS_ACCESS_KEY_ID=YOUR_AWS_ID_HERE
    ssh -t dokku@yourservername.com dokku config:set health_facilities DJANGO_AWS_SECRET_ACCESS_KEY=YOUR_AWS_SECRET_ACCESS_KEY_HERE
    ssh -t dokku@yourservername.com dokku config:set health_facilities DJANGO_AWS_STORAGE_BUCKET_NAME=YOUR_AWS_S3_BUCKET_NAME_HERE
    ssh -t dokku@yourservername.com dokku config:set health_facilities DJANGO_MAILGUN_API_KEY=YOUR_MAILGUN_API_KEY
    ssh -t dokku@yourservername.com dokku config:set health_facilities DJANGO_MAILGUN_SERVER_NAME=YOUR_MAILGUN_SERVER
    ssh -t dokku@yourservername.com dokku run health_facilities python manage.py migrate
    ssh -t dokku@yourservername.com dokku run health_facilities python manage.py createsuperuser
...
application stacks on one host, remove the port setting entirely and add `VIRTUAL_HOST=abugasav.io` to your env file.
This pass all incoming requests on `nginx-proxy` to the nginx service your application is using.

.. _nginx-proxy: https://github.com/jwilder/nginx-proxy

Postgres is saving its database files to `/data/health_facilities/postgres` by default. Change that if you wan't
...
If you are using `supervisor`, you can use this file as a starting point::
    ...
    [program:health_facilities]
    command=docker-compose up
    directory=/path/to/health_facilities
...
Place it in `/etc/supervisor/conf.d/health_facilities.conf` and run::

    supervisorctl reread
    supervisorctl start health_facilities
